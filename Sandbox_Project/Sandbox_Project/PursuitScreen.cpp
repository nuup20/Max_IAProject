...
void CPursuitScreen::init()
{
	...
	_newR = new CRendObject();
	_newR->init();
	_newR->m_name = "Finish Line";
	_newR->setPosition(1000, 100);
	_newR->setColor(255, 255, 255, 255);
	_newR->setSpriteDirectory("gameResources/sprites/spr_meta_01.png");
	m_world.addGameObject(_newR);
	...
	_newGO->addNewTarget(_newR, BOIDTARGET::kSeek);
	...
	_newGO->addNewTarget(objective, BOIDTARGET::kPursuit);
